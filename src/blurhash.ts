 ...
 *
 * Ships only when imported from `@cabraham/react-smart-loader/blurhash`, keeping it out of the
...
function decode83(str: string): number {
  ...
  for (const c of str) {
    ...
    if (i === -1) throw new Error(`@cabraham/react-smart-loader: invalid BlurHash character "${c}"`);
    value = value * 83 + i;
  }
  return value;
}
...
export function decodeBlurHash(
  ...
  if (hash.length < 6) throw new Error('@cabraham/react-smart-loader: BlurHash too short');
  ...
  if (hash.length !== expected) {
    throw new Error(`@cabraham/react-smart-loader: BlurHash length mismatch (expected ${expected})`);
  }
